(function (root) {
  ...
  // One U.S. broadcast clock for every viewer worldwide.
  // Channel UIs may convert these absolute timestamps to the viewer's local time.
  ...
  function movieRuntimeSeconds(movie) {
    return Math.max(60, Math.min(BLOCK_SECONDS, Math.floor(Number(movie && movie.runtimeSeconds) || 6000)));
  }
  ...
  function breakDurationSeconds(commercials, breakIndex) {
    ...
    let seconds = 0;
    for (let spot = 0; spot < SPOTS_PER_BREAK; spot += 1) {
  ...
  function fittedBreaks(movie, commercials) {
    const runtime = movieRuntimeSeconds(movie);
    let used = runtime;
    const selected = [];
    BREAK_AFTER_CONTENT_SECONDS.filter(n => n < runtime).forEach((boundary, index) => {
      const duration = breakDurationSeconds(commercials, index);
      if (used + duration <= BLOCK_SECONDS) {
        selected.push({boundary, index, duration});
        used += duration;
      }
    });
    return selected;
  ...
  function stationDurationSeconds() {
    // Fixed television grid: every movie owns exactly two hours.
    return BLOCK_SECONDS;
  }

  function masterCatalog(fallback) {
    const candidates = [root.HERMIT_CATALOG, root.STAR_LAUNCHER_CATALOG];
  ...
  function createDaySchedule(nowMs, catalog) {
    ...
    const passed = (Array.isArray(catalog) ? catalog : []).filter(movie => movie && movie.cleared && movie.videoId && movieRuntimeSeconds(movie) <= BLOCK_SECONDS);
    const allowedIds = new Set(passed.map(movie => movie.videoId));
    const master = masterCatalog(catalog).filter(movie => movie && movie.cleared && movie.videoId && movieRuntimeSeconds(movie) <= BLOCK_SECONDS);
    if (!master.length) throw new Error("No playable full movies are configured.");

    const todayKey = dateKey(nowMs);
    let shuffled = seededShuffle(master, todayKey);
    const previous = seededShuffle(master, dateKey(midnightMs - 1000));
    ...
    return Array.from({length: 12}, (_, index) => {
      const baseMovie = shuffled[index % shuffled.length];
      const movie = allowedIds.size && !allowedIds.has(baseMovie.videoId) ? {...baseMovie, cleared:false} : baseMovie;
  ...
  function createSegments(block, commercials) {
    ...
    const selectedBreaks = fittedBreaks(block.movie, commercials);
    const boundaries = [0, ...selectedBreaks.map(item => item.boundary), runtime];
    ...
    for (let i = 0; i < boundaries.length - 1; i += 1) {
      ...
      if (i < selectedBreaks.length) {
        for (let spot = 0; spot < SPOTS_PER_BREAK; spot += 1) {
          ...
          if (!pushSegment({kind:"commercial", title:ad.title || "Station break", videoId:ad.videoId || "", cleared:!!ad.cleared, sourceStart:0}, Math.max(1, Math.floor(Number(ad.durationSeconds) || DEFAULT_SPOT_SECONDS)))) break outer;
        }
      }
    }

    if (stationOffset < BLOCK_SECONDS) {
      pushSegment({kind:"station", title:"Next movie starts at the next two-hour slot", videoId:"", cleared:true, sourceStart:0}, BLOCK_SECONDS - stationOffset);
    }
    return segments;
  }
  ...
  function resolve(nowMs, schedule, commercials) {
    ...
    for (let i = segmentIndex + 1; segment.kind === "commercial" && i < segments.length && segments[i].kind === "commercial"; i += 1) movieReturnsIn += segments[i].duration;
    return {
